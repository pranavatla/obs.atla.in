...
def alb(ref, metric, stat, label, hide=False, name="${alb}", region="${region}", period=300):
    # The LoadBalancer dimension is app/<name>/<id>; searching by name avoids hard-coding the id.
    # FILL(…, 0): the load balancer publishes error counts only when errors happen.
    search = f"SEARCH('{{AWS/ApplicationELB,LoadBalancer}} MetricName=\"{metric}\" \"{name}\"', '{stat}', {period})"
    expr = f"FILL(SUM({search}), 0)" if stat == "Sum" else search
    return cw_search(ref, region, expr, label, hide)
...
def traffic(lay):
    ...
    lay.line([
        ...
        (stat("App 4xx responses", "Requests the app answered with a 4xx status (bad input, not found)." + ALB_SRC,
              [alb("A", "HTTPCode_Target_4XX_Count", "Sum", "4xx")], "short", calc="sum", decimals=0), 6),
    ...
    for cfg in DOMAINS:
        ...
        if "distribution" in cfg:
            ...
            err5 = stat("5xx responses", f"App and load balancer 5xx responses for {cfg['domain']}." + src,
                        [cw_search("A", cfg["region"],
                                   alb("X", "HTTPCode_Target_5XX_Count", "Sum", "", **kw)["expression"] + " + "
                                   + alb("Y", "HTTPCode_ELB_5XX_Count", "Sum", "", **kw)["expression"], "5xx")],
                        "short", ZERO_IS_GOOD, calc="sum", decimals=0)
